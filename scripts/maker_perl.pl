...
my $fn_file_out = join(".", "P6_post_ENA_trans_awk_fasta", "sh");
...
foreach my $line (<FN_FILE_LIST>) {
       ...
       # my $sub_name_num = substr($list[0], -1);
        ...
        my $command_line0 = "/scratch/x2304a03/P6/post_data/trans_awk_fasta.sh $sub_name_srr >> ENA_trans_awk_fasta_0526.log 2>&1\n";
