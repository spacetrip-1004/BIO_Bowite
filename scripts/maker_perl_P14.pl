...
my $fn_file_out = join(".", "P14_ENA_trans_awk_fasta", "sh");
...
foreach my $line (<FN_FILE_LIST>) {
        ...
        my $ID = $list[0];
        my $sub_name_srr = substr($list[0], 3, 7);
       # my $sub_name_num = substr($list[0], -1);

        my $command_line0 = "sh /scratch/e1412a01/P14/ENA_P14_pre_data/trans_awk_fasta.sh $sub_name_srr >> P14_ENA_trans_awk_fasta_0603.log 2>&1\n";
